...
export class ToolHandlers {
    ...
    tools;
    ensureConnection;
    constructor(db, tools, ensureConnection) {
        this.db = db;
        this.tools = tools;
            ...
            switch (request.params?.name) {
                ...
                case API_TOOLS.CREATE_COLLECTION: {
                    ...
                    try {
                        ...
                        const collection = await this.db.createCollection(args.name, options);
                ...
                case API_TOOLS.BACKUP: {
                    ...
                    try {
                        const results = [];
                        async function backupCollection(db, outputDir, collection, docLimit) {
                            try {
                ...
                case 'flexible_recent_articles': {
                    const { limit = 10, offset = 0, sortBy = 'default.epoch_time', sortOrder = 'desc', detail = 'summary', withRelated = [], groupBy, projection } = request.params.arguments;
                    let proj = 'article';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': article.' + f).join(', ')} }`;
                    }
                    ...
                    const cursor = await this.db.query(`FOR article IN Article SORT article.${sortBy} ${sortOrder.toUpperCase()} LIMIT @offset, @limit RETURN ${proj}`, { limit, offset });
                ...
                case 'flexible_search_articles_by_category': {
                    const { category, subcategory, limit = 10, offset = 0, sortBy = 'default.epoch_time', sortOrder = 'desc', detail = 'summary', withRelated = [], groupBy, projection } = request.params.arguments;
                    let filter = '@category IN article.category';
                    if (subcategory)
                        filter += ' && @subcategory IN article.subcategory';
                    let proj = 'article';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': article.' + f).join(', ')} }`;
                    }
                ...
                case 'flexible_search_articles_by_author': {
                    const { author, limit = 10, offset = 0, sortBy = 'default.epoch_time', sortOrder = 'desc', detail = 'summary', withRelated = [], groupBy, projection } = request.params.arguments;
                    let proj = 'article';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': article.' + f).join(', ')} }`;
                    }
                    ...
                    const cursor = await this.db.query(`FOR article IN Article FILTER article.author == @author SORT article.${sortBy} ${sortOrder.toUpperCase()} LIMIT @offset, @limit RETURN ${proj}`, { author, limit, offset });
                    let result = await cursor.all();
                    if (groupBy) {
                        result = result.reduce((acc, item) => {
                            const key = item[groupBy];
                ...
                case 'flexible_fulltext_search_articles': {
                    const { query: search, limit = 10, offset = 0, detail = 'summary', withRelated = [], groupBy, projection } = request.params.arguments;
                    let proj = 'article';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': article.' + f).join(', ')} }`;
                    }
                    ...
                    const cursor = await this.db.query(`FOR article IN Article FILTER CONTAINS(LOWER(article.default.title), LOWER(@search)) || CONTAINS(LOWER(article.default_summary), LOWER(@search)) || (HAS(article, 'description') && CONTAINS(LOWER(article.description), LOWER(@search))) LIMIT @offset, @limit RETURN ${proj}`, { search, limit, offset });
                    let result = await cursor.all();
                    if (groupBy) {
                        result = result.reduce((acc, item) => {
                            const key = item[groupBy];
                ...
                case 'flexible_article_by_key': {
                    const { key, detail = 'full', withRelated = [], projection } = request.params.arguments;
                    let proj = 'article';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': article.' + f).join(', ')} }`;
                    }
                ...
                case 'flexible_list_all_categories': {
                    const { limit = 100, offset = 0, article_key, groupBy } = request.params.arguments;
                ...
                case 'flexible_articles_by_entity': {
                    const { entity, limit = 10, offset = 0, detail = 'summary', withRelated = [], groupBy, projection } = request.params.arguments;
                    let proj = 'article';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': article.' + f).join(', ')} }`;
                    }
                    ...
                    const cursor = await this.db.query(`LET entityDoc = FIRST(FOR e IN Entity FILTER e._key == @entity OR e.name == @entity RETURN e)
                         ...
                         SORT article.default.epoch_time DESC LIMIT @offset, @limit RETURN ${proj}`, { entity, limit, offset });
                    let result = await cursor.all();
                    if (groupBy) {
                        result = result.reduce((acc, item) => {
                            const key = item[groupBy];
                ...
                case 'flexible_articles_by_date_range': {
                    const { start_epoch, end_epoch, limit = 10, offset = 0, detail = 'summary', withRelated = [], groupBy, projection } = request.params.arguments;
                    let proj = 'article';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': article.' + f).join(', ')} }`;
                    }
                    ...
                    const cursor = await this.db.query(`FOR article IN Article FILTER article.default.epoch_time >= @start_epoch && article.default.epoch_time <= @end_epoch SORT article.default.epoch_time DESC LIMIT @offset, @limit RETURN ${proj}`, { start_epoch, end_epoch, limit, offset });
                    let result = await cursor.all();
                    if (groupBy) {
                        result = result.reduce((acc, item) => {
                            const key = item[groupBy];
                ...
                case 'flexible_document_by_key': {
                    ...
                    let proj = 'doc';
                    if (projection && projection.length > 0) {
                        proj = `{ ${projection.map((f) => f + ': doc.' + f).join(', ')} }`;
                    }
                    else if (detail === 'minimal') {
                ...
                case 'flexible_documents_by_date_range': {
                    ...
                    const cursor = await this.db.query(`FOR doc IN Document FILTER doc.epoch_published >= @start_epoch && doc.epoch_published <= @end_epoch SORT doc.epoch_published DESC LIMIT @offset, @limit RETURN ${proj}`, { start_epoch, end_epoch, limit, offset });
                    let result = await cursor.all();
                    if (groupBy) {
                        result = result.reduce((acc, item) => {
                            const key = item[groupBy];
                ...
                case 'flexible_fulltext_search_documents': {
                    ...
                    const cursor = await this.db.query(`FOR doc IN Document FILTER CONTAINS(LOWER(doc.title), LOWER(@search)) || CONTAINS(LOWER(doc.description), LOWER(@search)) || CONTAINS(LOWER(doc.content), LOWER(@search)) LIMIT @offset, @limit RETURN ${proj}`, { search, limit, offset });
                    let result = await cursor.all();
                    if (groupBy) {
                        result = result.reduce((acc, item) => {
                            const key = item[groupBy];
                ...
                case 'flexible_list_document_authors': {
                    const { limit = 100, offset = 0, document_key, groupBy } = request.params.arguments;
                    let query, bindVars;
                    if (document_key) {
                ...
                case 'get_document_edges': {
                    const { document_id, limit = 20 } = request.params.arguments;
                    ...
                    for (const coll of edgeCollections) {
                        const cursor = await this.db.query(`FOR edge IN ${coll} FILTER edge._from == @docId OR edge._to == @docId LIMIT @limit RETURN MERGE(edge, { _edgeCollection: '${coll}' })`, { docId, limit });
...
async function getCrlrRelatedArticlesUnsetHandler(args) {
...
async function getCrlrRelatedArticlesHandler(args) {
    const { articleKey, graphDepth, edgeCollectionName, simThreshold, dbUrl } = args;
    const db = new Database({ url: dbUrl });
    const adb = db.database('newsDB2022');
    adb.useBasicAuth('root', 'i-0172f1f969c7548c4');
...
async function getPathRelatedArticlesUnsetHandler(args) {
...
async function getPathRelatedArticlesHandler(args) {
...
async function getRelatedArticlesGraphHandler(args) {
...
async function getCrlrRelatedDocsHandler(args) {
...
async function getPathRelatedDocsHandler(args) {
...
export { getCrlrRelatedArticlesUnsetHandler, getCrlrRelatedArticlesHandler, getPathRelatedArticlesUnsetHandler, getPathRelatedArticlesHandler, getRelatedArticlesGraphHandler, getCrlrRelatedDocsHandler, getPathRelatedDocsHandler };
