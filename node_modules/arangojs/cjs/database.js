...
function coerceTransactionCollections(collections) {
    ...
    if (Array.isArray(collections)) {
        return { write: collections.map(collection_js_1.collectionToString) };
    }
    if ((0, collection_js_1.isArangoCollection)(collections)) {
        return { write: (0, collection_js_1.collectionToString)(collections) };
    }
    const cols = {};
    if (collections) {
        ...
        if (collections.read) {
            cols.read = Array.isArray(collections.read)
                ? collections.read.map(collection_js_1.collectionToString)
                : (0, collection_js_1.collectionToString)(collections.read);
        }
        if (collections.write) {
            cols.write = Array.isArray(collections.write)
                ? collections.write.map(collection_js_1.collectionToString)
                : (0, collection_js_1.collectionToString)(collections.write);
        }
        if (collections.exclusive) {
            cols.exclusive = Array.isArray(collections.exclusive)
                ? collections.exclusive.map(collection_js_1.collectionToString)
                : (0, collection_js_1.collectionToString)(collections.exclusive);
        }
    }
    return cols;
}
...
class Database {
    ...
    constructor(configOrDatabase = {}, name) {
        if (isArangoDatabase(configOrDatabase)) {
            ...
            const { databaseName, ...options } = typeof config === "string" || Array.isArray(config)
                ...
                : config;
            this._connection = new connection_js_1.Connection(options);
     ...
     */
    route(path, headers) {
        return new route_js_1.Route(this, path, headers);
    }
    ...
    async availability(graceful = false) {
        try {
            ...
            if (graceful)
                return false;
            if (((0, error_js_1.isArangoError)(e) || e instanceof error_js_1.HttpError) && e.code === 503) {
                return false;
            }
            throw e;
        }
    ...
    async exists() {
        try {
            ...
            if ((0, error_js_1.isArangoError)(err) && err.errorNum === codes_js_1.DATABASE_NOT_FOUND) {
                return false;
            }
            throw err;
        }
    ...
    collection(collectionName) {
        ...
        if (!this._collections.has(collectionName)) {
            this._collections.set(collectionName, new collection_js_1.Collection(this, collectionName));
        }
    ...
    async createEdgeCollection(collectionName, options) {
        return this.createCollection(collectionName, {
            ...options,
            type: collection_js_1.CollectionType.EDGE_COLLECTION,
        });
    }
    /**
    ...
    graph(graphName) {
        if (!this._graphs.has(graphName)) {
            this._graphs.set(graphName, new graph_js_1.Graph(this, graphName));
        }
    ...
    view(viewName) {
        if (!this._views.has(viewName)) {
            this._views.set(viewName, new view_js_1.View(this, viewName));
        }
    ...
    analyzer(analyzerName) {
        if (!this._analyzers.has(analyzerName)) {
            this._analyzers.set(analyzerName, new analyzer_js_1.Analyzer(this, analyzerName));
        }
    ...
    getUserAccessLevel(username, { database, collection }) {
        const databaseName = isArangoDatabase(database)
            ? database.name
            : (database ??
                ((0, collection_js_1.isArangoCollection)(collection)
                    ? collection._db.name
                    : this._name));
        const suffix = collection
            ? `/${encodeURIComponent((0, collection_js_1.isArangoCollection)(collection) ? collection.name : collection)}`
            : "";
        return this.request({
    ...
    transaction(transactionId) {
        return new transaction_js_1.Transaction(this, transactionId);
    }
    beginTransaction(collections, options = {}) {
        ...
        }, (res) => new transaction_js_1.Transaction(this, res.parsedBody.result.id));
    }
    ...
    query(query, bindVars, options = {}) {
        if ((0, aql_js_1.isAqlQuery)(query)) {
            options = bindVars ?? {};
            bindVars = query.bindVars;
            query = query.query;
        }
        else if ((0, aql_js_1.isAqlLiteral)(query)) {
            query = query.toAQL();
        }
        ...
        return this.request({
            ...
            retryOnConflict,
            timeout,
        }, (res) => new cursor_js_1.BatchedArrayCursor(this, res.parsedBody, res.arangojsHostUrl, allowDirtyRead).items);
    }
     ...
     */
    listFunctions() {
        return this.request({ path: "/_api/aqlfunction" }, (res) => res.parsedBody.result);
    }
    /**
    ...
    async createJob(callback) {
        ...
        return new job_js_1.Job(this, jobId, (res) => {
            onResolve(res);
            return eventualResult;
        }, (e) => {
            onReject(e);
     ...
     */
    job(jobId) {
        return new job_js_1.Job(this, jobId);
    }
    /**
